...
php -d memory_limit=-1 vendor/bin/rector process bin src --config packages/config-transformer/build/config/config-downgrade.php --working-dir $BUILD_DIRECTORY --ansi
php -d memory_limit=-1 vendor/bin/rector process vendor --config packages/config-transformer/build/config/config-downgrade.php --working-dir $BUILD_DIRECTORY --ansi
